# GPT Image 2.0 E-commerce Product Prompts

This file focuses on product hero shots, campaign visuals, detail shots, and lifestyle e-commerce imagery.

## White Background Hero Shots

```text
Create a clean e-commerce hero image of a frosted glass skincare serum bottle with a brushed silver dropper cap on a pure white background. Front-facing centered composition, soft even studio lighting, very subtle grounding shadow, crisp edges, accurate proportions, premium material detail, commercial catalog quality, no text, no extra props, no reflections that distract from the product.
```

```text
Generate a premium catalog image of a rectangular perfume bottle made of clear glass with a matte black cap, isolated on a pure white background. Symmetrical front view, controlled reflections, sharp label area, luxury e-commerce style, high realism, no extra bottles, no decoration, no floating artifacts, no watermark.
```

```text
Draw a pair of matte white wireless earbuds with their charging case, arranged neatly on a pure white background for an online store hero image. Front three-quarter angle, soft shadow, crisp silhouette, realistic plastic texture, premium consumer electronics photography, no cables, no text, no duplicated earbuds.
```

## Premium Advertising Visuals

```text
Generate a luxury skincare advertising image featuring a frosted glass serum bottle on layered travertine stone blocks. Warm beige and soft gray palette, diffused daylight from the upper left, elegant shadows, premium beauty brand feel, sharp focus on bottle texture, minimal composition, no flowers, no splashes, no random accessories.
```

```text
Create a high-end perfume campaign image of a matte black bottle with an embossed silver cap placed on a dark volcanic stone pedestal. Controlled studio lighting, charcoal and silver palette, subtle rim light, premium luxury branding tone, crisp edges, rich material rendering, no extra props, no text artifacts.
```

```text
Draw a luxury watch advertisement showing a brushed steel wristwatch resting on a black stone surface with a few precise droplets of water. Dramatic but controlled lighting, deep blacks, silver highlights, sharp crystal reflections, premium Swiss watch campaign aesthetic, no extra jewelry, no duplicated hands, no clutter.
```

## Lifestyle Scenes

```text
Generate a lifestyle product image of a matte premium coffee bag on a warm walnut table beside scattered roasted coffee beans and a folded linen cloth. Morning window light, soft natural shadows, editorial food photography style, warm brown and cream palette, clean composition, no coffee cup, no spoon, no duplicate packaging.
```

```text
Create a lifestyle advertising image of a luxury scented candle on a soft beige bedside table next to a closed linen book and a ceramic tray. Late afternoon sunlight, calm warm shadows, premium home fragrance branding style, realistic wax and glass texture, minimal atmosphere, no flowers, no messy bedroom background.
```

## Detail Shots

```text
Create a macro-style product detail image of a premium leather handbag, focusing on the grain texture, stitching, edge paint, and brushed metal buckle. Soft directional studio light, luxury fashion e-commerce detail shot, sharp focus, rich tactile quality, no hands, no background clutter, no distorted hardware.
```

```text
Generate a close-up detail shot of a running shoe sole, emphasizing traction pattern, foam texture, and engineered grooves. Bright clean studio background, controlled lighting, technical yet premium sports-brand aesthetic, crisp detail, no dirt, no person, no exaggerated deformation.
```

## Social Ad Visuals

```text
Create a square social ad visual for a luxury serum bottle floating above a warm beige gradient background with soft shadow underneath. Centered composition, premium beauty campaign aesthetic, clean negative space for future copy placement, refined reflections, no text, no extra props, no clutter.
```

```text
Generate a vertical 4:5 product campaign image of a matte black perfume bottle against a dark charcoal gradient background with subtle haze. Dramatic top light, elegant reflections, strong focal point, premium luxury branding tone, clean negative space above and below for ad copy, no duplicate products, no decorative distractions.
```

## Web-inspired Additions II

```text
Create a luxury fragrance ad with an amber glass perfume bottle on a cream stone pedestal. Soft haze, editorial negative space, warm amber and deep olive palette, premium minimalist beauty-campaign feel, no extra props.
```

```text
Generate a clean beauty hero image showing a refillable skincare bottle set on a pale beige seamless background. Soft frontal light, restrained palette, sustainable premium packaging focus, refined DTC brand aesthetic.
```

```text
Create a luxury still life of a brushed steel watch and a dark brown leather wallet on a charcoal stone slab. Controlled side light, mature understated palette, premium menswear-accessory ad tone.
```

```text
Generate a performance running shoe ad with the sneaker floating above a light gray backdrop. Subtle shadow below, crisp mesh, foam, and outsole detail, international sports-brand commercial polish.
```

```text
Create a minimal haircare campaign with off-white shampoo and conditioner bottles on pale travertine platforms. Soft daylight, slight moisture in the air, cream, gray, and misty blue palette, clean premium beauty look.
```

```text
Generate a premium chocolate gift-box ad showing a deep brown box slightly open with a few chocolates neatly arranged nearby. Warm spotlighting, restrained gold accents, high-end seasonal food campaign aesthetic.
```

```text
Create an ultra-clean home-appliance hero visual with a white air purifier in a modern living-room corner. Soft daylight through curtains, generous negative space, subtle luxury tech-brand feel.
```

```text
Generate a minimalist jewelry campaign image with a single gold ring standing upright against a cream background. Focused top light, crisp highlight on the edge, elegant shadow, collector-grade luxury mood.
```

```text
Create a premium coffee machine lifestyle ad with a matte black espresso machine on a minimalist kitchen counter. Morning slant light, controlled reflections, very few supporting props, high-end home appliance campaign quality.
```

```text
Generate a social-ad cover image for a three-piece skincare set floating over a warm beige gradient background. Soft shadow underneath, centered composition, clean negative space for marketing copy, polished campaign finish.
```

## Category-specific Additions

```text
Create a high-end fashion ecommerce hero with a model wearing an ivory coat inside a minimal architectural space. Clean background, crisp fabric texture, premium apparel-homepage mood.
```

```text
Generate a flat-lay apparel product image of a premium knit sweater on a pale gray backdrop. Natural folds, visible texture, clean catalog styling.
```

```text
Create a jewelry earring ad with a pair of gold earrings floating against a dark background. Focused top light, crisp edge highlights, luxurious but minimal.
```

```text
Generate a close-up necklace detail image focusing on diamond facets, chain texture, and skin-contact realism. Soft blur behind, premium jewelry-detail-page finish.
```

```text
Create a home-fragrance and ceramic still life with a candle, diffuser bottle, and ceramic tray on a wooden table. Warm lifestyle light, calm premium atmosphere.
```

```text
Generate a sofa lifestyle campaign with a cream-colored couch in a minimalist living room. Afternoon sunlight, long shadows, elevated but livable home-brand tone.
```

```text
Create a premium bottled tea ad with a glass cold-brew bottle on pale stone, a few tea leaves nearby, light vapor, and a fresh restrained wellness aesthetic.
```

```text
Generate a high-end olive-oil packaging visual with the bottle and paper box placed side by side against a soft beige background. Clean label, editorial food-brand polish.
```

```text
Create a holiday dessert gift-box ad with the box open to reveal neatly arranged sweets. Warm golden light, deep red and cream accents, festive premium campaign feel.
```

```text
Generate a minimal home-decor catalog shot with one sculptural vase centered on a pale surface. Clean shadow, strong silhouette, design-brand directory quality.
```
